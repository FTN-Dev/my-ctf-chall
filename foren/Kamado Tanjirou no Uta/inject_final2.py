...
# durasi khusus untuk flag (atur sesuai kebutuhan agar teks full terlihat)
FLAG_DURATION = 15.0   # detik
...
def text_to_image(text, width, height, font_size=48):
    ...
    # kalau teks lebih panjang dari width, kecilkan font_size otomatis
    if w > width:
        scale = width / (w + 1e-9)
...
def make_flag_audio(flag_text, sr=22050, duration=10.0):
    ...
    img = text_to_image(flag_text, time_frames, 256, font_size=64)
